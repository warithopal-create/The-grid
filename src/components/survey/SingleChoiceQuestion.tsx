...
export default function SingleChoiceQuestion({ question, answer, otherText, onAnswer, onOtherText }: Props) {
        ...
        return (
          <motion.button
            ...
            className={`w-full group relative overflow-hidden rounded-xl border transition-all duration-300 ${
              isSelected
                ? 'border-emerald-400/50 bg-emerald-400/10 shadow-lg shadow-emerald-400/5'
            ...
            {option.hasOtherInput && isSelected && (
              ...
              >
                <input
                  ...
                  className={`w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white placeholder-white/30 focus:outline-none focus:border-emerald-400/50 focus:ring-1 focus:ring-emerald-400/20 transition-all ${isRTL ? 'text-right' : 'text-left'}`}
                  dir={isRTL ? 'rtl' : 'ltr'}
                  autoFocus
                />
              </motion.div>
